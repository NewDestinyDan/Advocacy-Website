...
def dhs_results_to_df(dhs_results):
    ...
    return dhs_results_df


def hra_results_to_df(hra_results):
    ...
    return hra_dv_df, hra_hasa_df


def hpd_results_to_df(hpd_results):
    ...
    return hpd_results_df


def dycd_results_to_df(dycd_results):
...
def ll37_get_open_data():
    ...
    dhs_results_df = dhs_results_to_df(dhs_results)
    hra_dv_df, hra_hasa_df = hra_results_to_df(hra_results)
    hpd_results_df = hpd_results_to_df(hpd_results)
    dycd_results_df = dycd_results_to_df(dycd_results)
